...
# Purpose:     THe Initial Model with Cofactor (e.g., Amax, PRI, MTCI) for 2012 data
#-------------------------------------------------------------------------------
import pymc
import numpy as np
import math
...
"""
Coding the Bayesian model for Logistic Leaf Gwoth model with pymc 2.3
"""

def FVT_Bayesian_Model_cofactor(Data,Lines,IDs,logindex_l,t0,splrep_tck_Y,rangm,KDEminmax,KDE_Object_Y,pprior):
    #------------------------------------------------------------------------------------------------------
    ...
    #---------------------------------------------------------------------------------------------------------------------------------------
    # Set the prior distributions for parameter br, a coefficient for revealing the photosynthesis activity during growth developmental process
    # --------------------------------------------------------------------------------------------------------------------------------------
    b_r = pymc.Normal(name='b_r', mu = pprior[6], tau = 1./pprior[7])
    ...
    for i in IDs:
        ...
        e_r_ij[i] = pymc.Normal(name='e_r_ij_%s' % i, mu = mu_r_i[l]+(b_r*logindex_l[l]), tau = tau_r_indv)
    ...
    #---------------------------------------------------------------------------------------------------------------------------------------
    # Set the prior distributions for parameter br, a coefficient for revealing the photosynthesis activity at maturity
    # --------------------------------------------------------------------------------------------------------------------------------------
    b_Lmax = pymc.Normal(name='b_Lmax', mu = pprior[8], tau = 1./pprior[9])
    ...
    for i in IDs:
        l = Data['Line'][Data['ID']==i][0]
        e_Lmax_ij[i] = pymc.Normal(name='e_Lmax_ij_%s' % i, mu = mu_Lmax_i[l]+(b_Lmax*logindex_l[l]), tau = tau_Lmax_indv)
    ...
    #--------------------------------------------------------------------------
    #  This function calculates the parameter L0_ij from e_L0_ij
    #--------------------------------------------------------------------------
