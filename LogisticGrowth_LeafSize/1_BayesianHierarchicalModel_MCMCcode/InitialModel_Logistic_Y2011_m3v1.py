...
# Purpose:     The Initial Model with slight modification for 2011 measurement
#                   data that exclude the final asymptotic phase
#-------------------------------------------------------------------------------
import pymc
import numpy as np
import math
...
def FVT_Bayesian_Model(Data,Lines,IDs,t0,splrep_tck_Y,rangm,KDEminmax,KDE_Object_Y,pprior):
    ...
    # ------------------------------------------------------------------------------------------------------
    prior_mu_Lmax_0_a = math.log(pprior[2])
    prior_mu_Lmax_0_b = math.log(pprior[3])
    tau_Lmax_alpha = 2.
    tau_Lmax_beta = 0.3

    mu_Lmax_G = pymc.Uniform(name='mu_Lmax_G',lower=prior_mu_Lmax_0_a, upper=prior_mu_Lmax_0_b)
